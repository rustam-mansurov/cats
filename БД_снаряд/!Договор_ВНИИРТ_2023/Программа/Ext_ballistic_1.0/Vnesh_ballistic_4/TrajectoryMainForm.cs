...
namespace Vnesh_ballistic
{
    public partial class TrajectoryMainForm : Form
    {
        ...
        {
            //string dir = VirtualFileSystem.ResourceDirectoryPath + "\\Polygon\\Data\\Trajectory2";
        ...
        public void StartInit()
        {
            ...
            res = VB.sn.LoadNormFromtxt(dir, ref mes);
            if (res != 0)
            {   MessageBox.Show("Загрузка нормативных коэффициентов. Ошибка: " + mes);
                System.Windows.Forms.Application.Exit(); }
            res = VB.m_ucl.LoadFromtxt(dir, ref mes);
            if (res != 0)
            ...
            {   MessageBox.Show("Загрузка реальных метеоданных. Ошибка: " + mes);
                System.Windows.Forms.Application.Exit();  }
            res = VB.g_ucl.LoadFromtxt(dir, ref mes);
            if (res != 0)
        ...
        public void ZapolnitForm0()
        {
          ...
          dataGridView1_NachUsloviya.Columns[0].DefaultCellStyle.Format = "0.###";
           ...
           chart1_Ozy.ChartAreas[0].AxisX.Minimum = 0;
            //geo
            textBox1_g.Text = VB.g_ucl.g_N0.ToString();
            textBox1_RadiusZemly.Text = VB.g_ucl.R_geo.ToString();
            textBox1_Gg.Text = VB.g_ucl.Gg.ToString();
            textBox1_delta2_g.Text = VB.g_ucl.delta2_g.ToString();
            textBox1_OmegaZemli.Text = VB.g_ucl.w_geo.ToString();
            //meteo
            textBox1_PN0.Text = VB.m_ucl.PN0.ToString();
            textBox1_roN0.Text = VB.m_ucl.roN0.ToString();
            textBox1_aN0.Text = VB.m_ucl.aN0.ToString();
            textBox1_tauN0.Text = VB.m_ucl.tauN0.ToString();
        }
        public void ZapolnitFormNUandParam()
        {
            ...
            checkBox_U_vrashen_zemli.Checked = VB.U_uchitivat_vrashenie_zemli;
            checkBox1_U_uchitivat_veter.Checked = VB.U_uchitivat_veter;
            if (VB.m_ucl.wind_type == 1) radioButton_w1.Checked = true;
            if (VB.m_ucl.wind_type == 2) radioButton_w2.Checked = true;
            if (VB.m_ucl.wind_type == 3) radioButton_w3.Checked = true;
            textBox_w10.Text = VB.m_ucl.w_veter10.ToString();
            textBox_aw.Text = VB.m_ucl.a_weter_grad.ToString();
        ...
        public int SchitatParamsAndNU()
        {
            try
            {
                ...
                //geo_ucl
                VB.g_ucl.g_N0 = Convert.ToDouble(textBox1_g.Text);
                VB.g_ucl.R_geo = Convert.ToDouble(textBox1_RadiusZemly.Text);
                VB.g_ucl.Gg = Convert.ToDouble(textBox1_Gg.Text);
                VB.g_ucl.delta2_g = Convert.ToDouble(textBox1_delta2_g.Text);
                VB.g_ucl.B_shirota = Convert.ToDouble(textBox1_B_shirota.Text) * Math.PI / 180;
                VB.g_ucl.w_geo = Convert.ToDouble(textBox1_OmegaZemli.Text);
                if (radioButton1_Zemlya_shar.Checked) VB.g_ucl.U1_FormaZemli = 1; else VB.g_ucl.U1_FormaZemli = 0;
                //snaryad
                ...
                //meteo_ucl
                VB.U_uchitivat_veter = checkBox1_U_uchitivat_veter.Checked;
                if (VB.U_uchitivat_veter)
                {
                    if (radioButton_w1.Checked) VB.m_ucl.wind_type = 1;
                    if (radioButton_w2.Checked) VB.m_ucl.wind_type = 2;
                    if (radioButton_w3.Checked) VB.m_ucl.wind_type = 3;
                    VB.m_ucl.w_veter10 = Convert.ToDouble(textBox_w10.Text);
                    VB.m_ucl.a_weter_grad = Convert.ToDouble(textBox_aw.Text);
                    VB.m_ucl.w_p = Convert.ToDouble(textBox_w_p.Text);
                }
                VB.eps_wind1 = 0; VB.eps_wind2 = 0;
                //nominal
                VB.m_ucl.PN0 = Convert.ToDouble(textBox1_PN0.Text);
                VB.m_ucl.roN0 = Convert.ToDouble(textBox1_roN0.Text);
                VB.m_ucl.aN0 = Convert.ToDouble(textBox1_aN0.Text);
                VB.m_ucl.tauN0 = Convert.ToDouble(textBox1_tauN0.Text);
                //real
                ...
                if (radioButton_Vkk.Checked) VB.b_ucl.Vkk = Convert.ToDouble(textBox_Vkk.Text); else VB.b_ucl.Vkk = 0;
        ...
        {           
            ...
            saveDialog.Filter = "Файл Excel|*.xls";
            saveDialog.InitialDirectory = GetDataDir() + "\\other";
            if (saveDialog.ShowDialog() == DialogResult.Cancel) return;
            this.Cursor = Cursors.WaitCursor;
            bool podrobno = podrobno_checkBox.Checked;
            res = VB.SaveTo_xls(saveDialog.FileName, podrobno, ref mes);
        ...
        private void button3_Click(object sender, EventArgs e)//сохранить в блокнот
        {
            ...
            if (saveDialog.ShowDialog() == DialogResult.Cancel) return;
            bool podrobno = podrobno_checkBox.Checked;
            res = VB.SaveTo_txt(saveDialog.FileName, podrobno, ref mes);
            if (res != 0) MessageBox.Show("Ошибка: " + mes);
            else MessageBox.Show("Данные успешно записаны!");
        }
        ...
        {
            VB.U_uchitivat_vrashenie_zemli = checkBox_U_vrashen_zemli.Checked;
        ...
        private void button1_OpenNU_and_params_Click(object sender, EventArgs e)//открыть из блокнота начальные условия и параметры
        {
            OpenFileDialog openDialog = new OpenFileDialog();
            openDialog.Filter = "Текстовый Файл|*.txt";
            openDialog.InitialDirectory = dir + "\\initialdata";
            if (openDialog.ShowDialog() == DialogResult.Cancel) return;
            res = VB.LoadNUandParam(openDialog.FileName, ref mes);
            if (res != 0)
            {
                MessageBox.Show("Ошибка: " + mes);
                return;
        ...
        private void button1_SaveNU_Click(object sender, EventArgs e)
        {
            ...
            if (saveDialog.ShowDialog() == DialogResult.Cancel) return;
            res = VB.SaveNUandParam(saveDialog.FileName, ref mes);
        }
        ...
        void LoadSnaryad(string f)
        {
            res = VB.sn.LoadRealFromtxt(f, ref mes);
            if (res != 0)
            {
            ...
            VB.b_ucl.teta0 = VB.sn.tet0;
            dataGridView1_NachUsloviya.Rows[5].Cells[0].Value = VB.sn.Vs0;
            dataGridView1_NachUsloviya.Rows[6].Cells[0].Value = VB.sn.tet0;
            VB.InitFromNU();//!!!
            string law = "";
            ...
            radioButton1_SvoyLaw.Checked = true;
        }
        ...
        {
            if (checkBox_mx_const.Checked) VB.MxConst = true;
            else VB.MxConst = false;
        }
        ...
        private void button_meteo_Click(object sender, EventArgs e)
        {
            ...
            if (openDialog.ShowDialog() == DialogResult.Cancel) return;
            res = VB.m_ucl.LoadRealFromtxt(openDialog.FileName, ref mes);
            if (res != 0) MessageBox.Show("Ошибка: " + mes);
        }
        ...
        private void radioPk()
        {
            ...
            textBox_Vkk.Enabled = radioButton_Vkk.Checked;
            if (!radioButton_tk.Checked) textBox_tk.Text = "";
            if (!radioButton_xk.Checked) textBox_xk.Text = "";
            if (!radioButton_Vkk.Checked) textBox_Vkk.Text = "";
        }
        ...
        private void radioButton_Vkk_CheckedChanged(object sender, EventArgs e)
        {
            if (radioButton_Vkk.Checked) radioPk();
        }
        ...
        {
            if (radioButton_atm_nom.Checked) VB.m_ucl.U2_nomin_real = 0;
            else VB.m_ucl.U2_nomin_real = 1;
        }
