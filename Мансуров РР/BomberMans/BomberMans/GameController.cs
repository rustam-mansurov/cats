    ...
    public class GameController
    {
        ...
        public void SetupRandomLevel(int mapSize)
        {
            ...
            for (int i = 0; i < MapSize; i++)
            {
                for (int j = 0; j < MapSize; j++)
                {
                    ...
                    if (i == 0 || j == 0 || i == MapSize - 1 || j == MapSize - 1 || hw <= spawner.HeavyWallSpawnProbability)
                    {
            ...
            int diamondsCount = 1)
        {
            ...
            while (true)
            {
                ...
                {
                    gameObjects.Add((i, j), new Diamond(i, j));
                    diamondsCount--;
                }
            }
        }
        ...
        private void SuperPowerShot(int i0, int j0, int di, int dj, string ip)
        {
            ...
            for (int k = 0; k < 2 * (Bomb.BasicPower + p.BombPowerModificator); k++)
            {
                ...
                var go = gameObjects[coords];
                var brokenObj = gameObjects.ContainsKey(coords) && go.BlockExplosion;
                ...
                if (broken > SuperPowerBonus.MaxPenetration || !go.Destructible && go.BlockExplosion)
                    break;
            }
        }
        ...
        private void MakeBombExplosion(Bomb b, string? owner = null)
        {
            ...
            foreach (var (dx, dy) in directions)
            {
                ...
                {
                    var go = gameObjects[(i, j)];
                    var stop = gameObjects.ContainsKey((i, j)) && go.BlockExplosion;
                    AddExplosion(i, j, owner);
